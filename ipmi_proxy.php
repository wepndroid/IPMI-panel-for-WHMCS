...
function ipmiProxyBuildKvmAutoLaunchPreambleJs(string $familyJs, string $planJs, string $pxJs, string $autoJs, string $dbgLit): string
{
    return '(function(){'
        ...
        . 'function kvmBugFinalizeRun(extra){try{if(!KVM_BUGLOG||!KVM_BUGLOG.run_id||!KVM_BUGLOG.ingest)return;var tok=kvmBugToken();if(!tok||tok.length!==64)return;var d=(extra!=null&&typeof extra==="object")?extra:{};var body=JSON.stringify({token:tok,run_id:String(KVM_BUGLOG.run_id||""),section:"FINAL",event:"kvm_run_finalize",detail:d});if(typeof navigator!=="undefined"&&navigator.sendBeacon){try{var bbF=new Blob([body],{type:"application/json"});if(navigator.sendBeacon(KVM_BUGLOG.ingest,bbF))return;}catch(eF1){}}try{if(typeof fetch!=="undefined"){fetch(KVM_BUGLOG.ingest,{method:"POST",headers:{"Content-Type":"application/json"},body:body,keepalive:true,credentials:"same-origin"});}}catch(eF2){}}catch(eF){}}'
        ...
        . 'try{if(typeof window.addEventListener==="function"){window.addEventListener("pagehide",function(){try{kvmBugFinalizeRun({reason:"pagehide"});}catch(_ph){}},true);}}catch(_peh){}'
...
function ipmiProxyBuildKvmRuntimeProgressHelpersJs(): string
{
    return ''
        ...
        . 'if(isRelay){try{window.__ipmi_kvm_ws_relay_ts=kvmNow();w.__ipmi_kvm_ws_relay_ts=kvmNow();window.__ipmi_kvm_ws_connect_attempted=(window.__ipmi_kvm_ws_connect_attempted||0)+1;_kvmDbg("ipmi_ws_relay_client_open",{url:String(u).substring(0,120),attempt:window.__ipmi_kvm_ws_connect_attempted||1});try{kvmBugSend("TRANSPORT","browser_ws_relay_connect_attempted",{attempt:window.__ipmi_kvm_ws_connect_attempted||1});}catch(_kbc){}}catch(_ts){}'
        . 'try{sock.addEventListener("open",function(){try{window.__ipmi_kvm_ws_last_msg_ts=kvmNow();window.__ipmi_kvm_ws_handshake_ok=(window.__ipmi_kvm_ws_handshake_ok||0)+1;window.__ipmi_kvm_ws_open_ts=kvmNow();_kvmDbg("ipmi_ws_relay_handshake_ok",{url:String(u).substring(0,120),ok_count:window.__ipmi_kvm_ws_handshake_ok||1});kvmBugSend("TRANSPORT","browser_ws_handshake_succeeded",{url:String(u).substring(0,96)});}catch(_o){}});}catch(_ol){}'
...
function ipmiProxyBuildIloHelperRuntimeJs(string $pxJs, string $dbgLit, ?array $kvmBuglog = null): string
{
    ...
    return '(function(){'
        ...
        . 'function kvmBugSend(section,event,detail){try{if(!KVM_BUGLOG||!KVM_BUGLOG.run_id||!KVM_BUGLOG.ingest)return;var tok=kvmBugToken();if(!tok||tok.length!==64)return;var body=JSON.stringify({token:tok,run_id:String(KVM_BUGLOG.run_id||""),section:String(section||"BROWSER"),event:String(event||""),detail:(detail!=null&&typeof detail==="object")?detail:{note:String(detail||"")}});if(typeof navigator!=="undefined"&&navigator.sendBeacon){try{var bb=new Blob([body],{type:"application/json"});if(navigator.sendBeacon(KVM_BUGLOG.ingest,bb))return;}catch(e1){}}try{if(typeof fetch!=="undefined"){fetch(KVM_BUGLOG.ingest,{method:"POST",headers:{"Content-Type":"application/json"},body:body,keepalive:true,credentials:"same-origin"});}}catch(e2){}}catch(e){}}'
        . 'function kvmBugFinalizeRun(extra){try{if(!KVM_BUGLOG||!KVM_BUGLOG.run_id||!KVM_BUGLOG.ingest)return;var tok=kvmBugToken();if(!tok||tok.length!==64)return;var d=(extra!=null&&typeof extra==="object")?extra:{};var body=JSON.stringify({token:tok,run_id:String(KVM_BUGLOG.run_id||""),section:"FINAL",event:"kvm_run_finalize",detail:d});if(typeof navigator!=="undefined"&&navigator.sendBeacon){try{var bbF=new Blob([body],{type:"application/json"});if(navigator.sendBeacon(KVM_BUGLOG.ingest,bbF))return;}catch(eF1){}}try{if(typeof fetch!=="undefined"){fetch(KVM_BUGLOG.ingest,{method:"POST",headers:{"Content-Type":"application/json"},body:body,keepalive:true,credentials:"same-origin"});}}catch(eF2){}}catch(eF){}}'
        . 'try{if(typeof window.addEventListener==="function"){window.addEventListener("pagehide",function(){try{kvmBugFinalizeRun({reason:"helper_pagehide"});}catch(_phh){}},true);}}catch(_pehh){}'
